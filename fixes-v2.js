// MAX TIME v2 — original v1 timer behavior + audio + portrait lock
(function(){
  ...
  // Exact boxing bell synthesis from original MAX TIME.
  function playBoxingBell(volumeMultiplier=1.0){
    try{
      ...
      [{freq:1050,amp:.35,decay:1.4},{freq:2100,amp:.18,decay:1.0},{freq:3150,amp:.10,decay:.7},{freq:4200,amp:.05,decay:.5},{freq:525,amp:.12,decay:1.2}].forEach(({freq,amp,decay})=>{
  ...
  // Copy v1 completion behavior: check a set -> auto timer unless it is the last set.
  window.toggleSet=function(ex,i){
    const sets=getSets(ex),was=sets[i].done;
    sets[i].done=!was;
    if(!was){
      const isLastSet=i===sets.length-1;
      if(!isLastSet){
        const cur=sets[i],next=sets[i+1];
        if(cur.load)next.load=cur.load;
        if(cur.reps)next.reps=cur.reps;
        if(cur.rir!==undefined)next.rir=cur.rir;
        startTimer(ex.rest,ex.n);
      }
    }
    save();render();
  };

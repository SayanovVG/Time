...
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(k=>Promise.all(k.filter(x=>x!==CACHE&&x.startsWith('two-cats-3d-v6-')).map(x=>caches.delete(x)))));self.clients.claim()});

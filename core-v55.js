...
(function(){
  ...
  function report(days){
    const cut=new Date();cut.setHours(0,0,0,0);cut.setDate(cut.getDate()-Math.max(1,days)+1);const after=d=>new Date(d+'T12:00:00').getTime()>=cut.getTime();
    const recent=(S.measurements||[]).filter(x=>x&&x.date&&after(x.date)).sort((a,b)=>a.date.localeCompare(b.date));
    const w=recent.filter(x=>Number.isFinite(+x.weight)),wa=recent.filter(x=>Number.isFinite(+x.waist));
    const lastWeight=a=>a.length?+a[a.length-1].weight:null,lastWa=a=>a.length?+a[a.length-1].waist:null;
    const wd=w.length>1?+(+w.at(-1).weight-+w[0].weight).toFixed(1):null,wad=wa.length>1?+(+wa.at(-1).waist-+wa[0].waist).toFixed(1):null;
    const food=Object.values(S.dailySummary||{}).filter(x=>x&&x.date&&after(x.date)).sort((a,b)=>a.date.localeCompare(b.date)),avg=k=>food.length?Math.round(food.reduce((s,x)=>s+(+x[k]||0),0)/food.length):0;
    let sets=0,dates=new Set();Object.entries(S.train||{}).forEach(([k,a])=>{const d=k.slice(0,10);if(!after(d))return;const c=(a||[]).filter(x=>x&&x.done).length;if(c){sets+=c;dates.add(d)}});
    return [`MAX TIME — отчёт за ${days} дней`,`Режим: рекомпозиция, 5 утренних силовых тренировок. MMA временно отложено.`,``,`ЗАМЕРЫ`,`Вес: ${lastWeight(w)??'нет данных'} кг${wd===null?'':`; изменение ${wd>0?'+':''}${wd} кг`}.`,`Талия: ${lastWa(wa)??'нет данных'} см${wad===null?'':`; изменение ${wad>0?'+':''}${wad} см`}.`,``,`ПИТАНИЕ`,food.length?`Заполнено дней: ${food.length}/${days}. Среднее: ${avg('cal')} ккал, Б ${avg('p')} г, Ж ${avg('f')} г, У ${avg('c')} г. Цель: 2450 ккал · Б180 · Ж75 · У264.`:'Нет сохранённых данных за период.',``,`ТРЕНИРОВКИ`,`Тренировочных дней: ${dates.size}. Выполненных рабочих подходов: ${sets}.`,``,`Проанализируй рекомпозицию, силовой прогресс, питание и восстановление без MMA. Приоритет: рост/сохранение сухой мышечной массы при постепенном снижении талии.`].join('\n')
  }
